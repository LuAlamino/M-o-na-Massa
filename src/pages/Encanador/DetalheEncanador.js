...
export default function DetalhesEncanador({ route, navigation }) {
  const { encanador } = route.params;
  ...
  useEffect(() => {
    const loadUserAndComments = async () => {
      ...
      // Carrega os comentários do encanador
      const comentariosSalvos = JSON.parse(await AsyncStorage.getItem(`comentarios_${encanador.id}`)) || [];
  ...
  const adicionarComentario = async () => {
    ...
    await AsyncStorage.setItem(`comentarios_${encanador.id}`, JSON.stringify(novaListaComentarios));
  ...
  // Função para excluir o encanador (apenas admin)
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            const novosCadastros = cadastros.filter((u) => u.id !== encanador.id);
            await AsyncStorage.setItem("users", JSON.stringify(novosCadastros));
            Alert.alert("Sucesso", "Encanador excluído.");
  ...
  return (
    <View style={styles.container}>
      <Image source={{ uri: encanador.profilePhoto }} style={styles.profilePhoto} />
      <Text style={styles.title}>{encanador.name}</Text>
      <Text style={styles.info}>E-mail: {encanador.email}</Text>
      <Text style={styles.info}>Idade: {encanador.age}</Text>
      <Text style={styles.info}>Experiência: {encanador.experience} anos</Text>
      <Text style={styles.info}>Telefone: {encanador.phone}</Text>
      <Text style={styles.info}>Profissão: {encanador.job}</Text>
