...
export default function DetalhesJardineiro({ route, navigation }) {
  const { jardineiro } = route.params;
  ...
  useEffect(() => {
    const loadUserAndComments = async () => {
      ...
      // Carrega os comentários do jardineiro
      const comentariosSalvos = JSON.parse(await AsyncStorage.getItem(`comentarios_${jardineiro.id}`)) || [];
  ...
  const adicionarComentario = async () => {
    ...
    await AsyncStorage.setItem(`comentarios_${jardineiro.id}`, JSON.stringify(novaListaComentarios));
  ...
  // Função para excluir o jardineiro (apenas admin)
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            const novosCadastros = cadastros.filter((u) => u.id !== jardineiro.id);
            await AsyncStorage.setItem("users", JSON.stringify(novosCadastros));
            Alert.alert("Sucesso", "Jardineiro excluído.");
  ...
  return (
    <View style={styles.container}>
      <Image source={{ uri: jardineiro.profilePhoto }} style={styles.profilePhoto} />
      <Text style={styles.title}>{jardineiro.name}</Text>
      <Text style={styles.info}>E-mail: {jardineiro.email}</Text>
      <Text style={styles.info}>Idade: {jardineiro.age}</Text>
      <Text style={styles.info}>Experiência: {jardineiro.experience} anos</Text>
      <Text style={styles.info}>Telefone: {jardineiro.phone}</Text>
      <Text style={styles.info}>Profissão: {jardineiro.job}</Text>
