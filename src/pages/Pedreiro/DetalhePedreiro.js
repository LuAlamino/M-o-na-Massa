...
export default function DetalhesPedreiro({ route, navigation }) {
  const { pedreiro } = route.params;
  ...
  useEffect(() => {
    const loadUserAndComments = async () => {
      ...
      // Carrega os comentários do pedreiro
      const comentariosSalvos = JSON.parse(await AsyncStorage.getItem(`comentarios_${pedreiro.id}`)) || [];
  ...
  const adicionarComentario = async () => {
    ...
    await AsyncStorage.setItem(`comentarios_${pedreiro.id}`, JSON.stringify(novaListaComentarios));
  ...
  // Função para excluir o pedreiro (apenas admin)
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            const novosCadastros = cadastros.filter((u) => u.id !== pedreiro.id);
            await AsyncStorage.setItem("users", JSON.stringify(novosCadastros));
            Alert.alert("Sucesso", "Pedreiro excluído.");
  ...
  return (
    <View style={styles.container}>
      <Image source={{ uri: pedreiro.profilePhoto }} style={styles.profilePhoto} />
      <Text style={styles.title}>{pedreiro.name}</Text>
      <Text style={styles.info}>E-mail: {pedreiro.email}</Text>
      <Text style={styles.info}>Idade: {pedreiro.age}</Text>
      <Text style={styles.info}>Experiência: {pedreiro.experience} anos</Text>
      <Text style={styles.info}>Telefone: {pedreiro.phone}</Text>
      <Text style={styles.info}>Profissão: {pedreiro.job}</Text>
