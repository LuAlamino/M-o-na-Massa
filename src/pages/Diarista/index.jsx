...
export default function Diarista({ navigation, route }) {
  ...
  // Carrega os diaristas e verifica o usuário logado ao abrir a tela
  useEffect(() => {
    carregarDiaristas();
    carregarUsuarioLogado(); // Carrega o usuário logado
  }, []);

  // Atualiza a lista de diaristas quando a tela recebe foco
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      carregarDiaristas(); // Recarrega os diaristas quando a tela recebe foco
    });
  ...
  // Carrega os diaristas
  const carregarDiaristas = async () => {
    const cadastros = await recuperarCadastros();
    const diaristasFiltrados = cadastros.filter(
      (cadastro) => cadastro.job === "Diarista"
    );
    setDiaristas(diaristasFiltrados);
  };
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            carregarDiaristas(); // Atualiza a lista de diaristas
          },
          style: "destructive",
        },
      ]
  ...
  // Filtra os diaristas com base na cidade selecionada
  const diaristasFiltrados =
    cidadeSelecionada === "Todas"
      ? diaristas
      : diaristas.filter((diarista) => diarista.city === cidadeSelecionada);

  // Navega para a tela de detalhes do diarista
  const verDetalhes = (diarista) => {
    navigation.navigate("DetalhesDiarista", {
      diarista,
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Diaristas 🧹🧼🧽</Text>
      ...
      {diaristasFiltrados.length > 0 ? (
        ...
        <Text style={styles.noResults}>
          Nenhum diarista {cidadeSelecionada !== "Todas" ? `em ${cidadeSelecionada}` : ""} cadastrado.
        </Text>
      )}
    </View>
  );
