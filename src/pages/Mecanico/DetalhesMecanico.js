...
export default function DetalhesMecanico({ route, navigation }) {
  ...
  useEffect(() => {
    const loadUserAndComments = async () => {
      ...
      // Carrega os comentários do mecânico
      const comentariosSalvos = JSON.parse(await AsyncStorage.getItem(`comentarios_${mecanico.id}`)) || [];
  ...
  const adicionarComentario = async () => {
    ...
    await AsyncStorage.setItem(`comentarios_${mecanico.id}`, JSON.stringify(novaListaComentarios));
  ...
  // Função para excluir o mecânico (apenas admin)
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            const novosCadastros = cadastros.filter((u) => u.id !== mecanico.id);
            await AsyncStorage.setItem("users", JSON.stringify(novosCadastros));
            Alert.alert("Sucesso", "Mecânico excluído.");
  ...
  return (
    <View style={styles.container}>
      <Image source={{ uri: mecanico.profilePhoto }} style={styles.profilePhoto} />
      <Text style={styles.title}>{mecanico.name}</Text>
      <Text style={styles.info}>E-mail: {mecanico.email}</Text>
      <Text style={styles.info}>Idade: {mecanico.age}</Text>
      <Text style={styles.info}>Experiência: {mecanico.experience} anos</Text>
      <Text style={styles.info}>Telefone: {mecanico.phone}</Text>
      <Text style={styles.info}>Profissão: {mecanico.job}</Text>
